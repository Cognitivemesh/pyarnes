...
## Code Audit (`pyarnes_bench.audit`)

In-tree code-graph + audit tool. No MCP server, no LLM dependency. Configuration lives in `pyproject.toml` under `[tool.pyarnes-audit]`.

| Task | Purpose |
|------|---------|
| `uv run tasks audit:build` | Parse the configured roots and persist `.pyarnes/audit/graph.json`. |
| `uv run tasks audit:show` | Print a human-readable summary of the persisted graph. |
| `uv run tasks audit:analyze` | God nodes, surprising connections, suggested questions. |
| `uv run tasks audit:check` | Detect unused files / exports / dependencies, circular imports, duplicate blocks, complexity hotspots, boundary violations, and feature-flag usage. Non-zero exit on HIGH findings — suitable for CI. |

All activity is logged via `pyarnes_core.observability` as JSONL on stderr. The persisted graph is the source of truth — query it weeks later without re-parsing source.
