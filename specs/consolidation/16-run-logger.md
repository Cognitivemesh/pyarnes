# pyarnes_swarm — Run Logger and Capture
...
> | **Tags** | observability, logging, runs |
...
> | **Depends on** | 15-bench-integrated-axes.md, 10-hook-integration.md |
> | **Extends** | 08-token-budget.md |
> | **Supersedes** | harness-run-logger.md |
> | **Read after** | 15-bench-integrated-axes.md |
> | **Read before** | 20-graph-package.md |
> | **Not owned here** | evaluation semantics — `ScoreResult` and `EvalResult` schemas (see `15-bench-integrated-axes.md`); external hook contract (see `10-hook-integration.md`); token counting (see `08-token-budget.md`) |
...
`ToolCallLogger` already writes JSONL traces, and `EvalSuite` already computes `EvalResult` rows, but there is no canonical persistent format that groups them into one run. `15-bench-integrated-axes.md` depends on:
...
`Cost` and `TokenUsage` are imported from `pyarnes_swarm.bench.burn.types` (same source as `EvalResult` in `15-bench-integrated-axes.md`).

#### `EvalResult` additive metadata

The base `EvalResult` schema is defined in `15-bench-integrated-axes.md`. This spec extends it with three optional fields so rows can be tied back to a run. This definition supersedes the baseline; spec 15 describes the pre-logger schema.
...
- !!! danger "Remote sink failures do not block local logging"
    Sink failures must not corrupt the local JSON files. Writes to disk succeed before attempting remote sync.
...
- `10-hook-integration.md` owns Claude Code hook behavior and the dev-time `.pyarnes/` files written by hooks.
- This spec owns per-run capture for runtime and evaluation flows.
- `15-bench-integrated-axes.md` may assume these types exist and use them in examples, but should not redefine them.
...
- `10-hook-integration.md` — Claude Code hook audit trail and dev-time `.pyarnes/` layout
- `15-bench-integrated-axes.md` — evaluation workflows that consume `RunReport` and `EvalRunLogger`

**Extends:**

- `08-token-budget.md` — token accounting inputs that populate `RunReport.tokens_in` and `tokens_out`
