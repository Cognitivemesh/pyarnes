# pyarnes_swarm — Bench (Evaluation Axes)
...
> | **Tags** | evaluation, benchmark, scoring |
...
> | **Depends on** | 07-swarm-api.md, 08-token-budget.md |
> | **Extends** | 16-run-logger.md, 22-evaluation-taxonomy.md |
...
> | **Read after** | 14-secrets.md |
> | **Read before** | 16-run-logger.md |
> | **Not owned here** | error taxonomy definitions (see `01-package-structure.md`); recovery semantics (see `07-swarm-api.md`); run persistence schema (see `16-run-logger.md`); model routing (see `06-model-router.md`); evaluation-taxonomy reference appendix (see `22-evaluation-taxonomy.md`); graph-based scorers (see `20-graph-package.md`) |
...
The `run_id`, `started_at`, and `finished_at` fields are additive; see `16-run-logger.md` for the extended schema that ties evaluation rows to a run.
...
> **Diagram:** [RACE four-dimension scoring](diagrams/15-race-scoring.html). Four dimensions (comprehensiveness · depth · instruction following · readability) feed into `internal_target` / `internal_reference`; the weighted aggregate is `final_score ∈ [0, 1]`.
...
> **Diagram:** [FACT pipeline](diagrams/15-fact-pipeline.html). Serial four-stage flow: extraction · dedup · per-claim verification · aggregate. Per-claim abstains when `sources[url]` is missing.
...
> **Prerequisite specs:** The worked examples below reference `RunReport`, `EvalRunLogger`, and `TursoSink` (defined in `specs/consolidation/16-run-logger.md`) and `IterationBudget` plus token-accounting rules (defined across `07-swarm-api.md` and `08-token-budget.md`). Read those specs for the type definitions before implementing these patterns.
...
| `execution_time` | `RunReport.wallclock_s` (see 16-run-logger.md) |
| `time_score` | adopter computes from `RunReport` (see 16-run-logger.md) |
...
- **Primitives:** an `IterationBudget` for shared step limits plus the token controls from `08-token-budget.md` wired through `LoopConfig`. Budget pressure surfaces as a clean stop or `UserFixableError` depending on the caller's policy.
- **Signal:** Surface `RunReport.wallclock_s`, `tokens_in + tokens_out`, `retries` in the evaluation row (see 16-run-logger.md). No new scorer needed — purely run-report analysis.

##### UC-D5 — Composition ablation (2³ matrix)

- **What:** Which subsystem contributes most to quality — guardrails? planning? retrieval?
- **Primitives:** run the same scenarios eight times with every combination of `{guardrails ∈ {on, off}} × {planner ∈ {on, off}} × {retrieval ∈ {subset_A, subset_B}}`. Each run gets its own `run_id`; `EvalRunLogger` (see 16-run-logger.md) writes one row per run into Turso (if configured) or per-run JSONL.
...
from pyarnes_swarm.capture import EvalRunLogger  # see 16-run-logger.md
from pyarnes_swarm.capture.sinks import TursoSink  # see 16-run-logger.md
...
`IterationBudget` caps shared loop steps; `FuzzyScorer` per-fact gives a coverage percentage; `RunReport` (spec 16) carries ops metrics to Turso for post-hoc dashboards.
...
| `Tier1Result.execution_time` | `RunReport.wallclock_s` (see 16-run-logger.md) |
...
| `TraceCollector → SQLite` (upstream) | `EvalRunLogger` + optional `TursoSink` (libSQL/async) (see 16-run-logger.md) |
