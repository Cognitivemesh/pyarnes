# pyarnes_swarm — Secrets and Credential Redaction
...
> | **Depends on** | 13-provider-config.md |
> | **Extends** | — |
> | **Supersedes** | — |
> | **Read after** | 13-provider-config.md |
> | **Read before** | 15-bench-integrated-axes.md |
> | **Not owned here** | provider config (see `13-provider-config.md`); transport (see `12-transport.md`); error taxonomy definitions (see `01-package-structure.md`); message-safety pipeline (see `11-message-safety.md`) |
...
> **Diagram:** [ChainedSecretStore fallback](diagrams/14-secret-fallback.html). Local-dev keychain hit and CI env-fallback paths over the same `get()` call.
