# pyarnes_swarm — Provider Config (Through Which Provider)
...
> | **Tags** | providers, configuration, models |
...
> | **Depends on** | 06-model-router.md, 12-transport.md |
> | **Extends** | 14-secrets.md |
> | **Supersedes** | — |
> | **Read after** | 12-transport.md |
> | **Read before** | 14-secrets.md |
> | **Not owned here** | model selection (see `06-model-router.md`); transport adapters / schema conversion (see `12-transport.md`); secrets storage (see `14-secrets.md`); evaluation cost calculation (see `15-bench-integrated-axes.md`) |
...
For streaming responses, implement `StreamingProviderTransport` (a sub-protocol of `ProviderTransport`; see `12-transport.md`). Non-streaming providers only need `ProviderTransport`.

By indexing parameters uniformly in the routing abstractions, the framework guarantees reliable token-budget measurements and threshold checks for diverse backends.
