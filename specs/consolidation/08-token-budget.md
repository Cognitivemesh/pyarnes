...
> | **Status** | active |
> | **Type** | core-runtime |
> | **Tags** | budget, tokens, cost, limits |
...
> | **Extends** | 06-model-router.md |
> | **Supersedes** | — |
> | **Read after** | 07-swarm-api.md |
> | **Read before** | 10-hook-integration.md |
> | **Not owned here** | runtime loop sequence (see `07-swarm-api.md`); model selection (see `06-model-router.md`); evaluation result schema (see `15-bench-integrated-axes.md`); run persistence (see `16-run-logger.md`); error taxonomy definitions (see `01-package-structure.md`) |
...
- [ ] Measure empty-message overhead with `acount_tokens()`
- [ ] Measure output-token error margin via P95 sampling on real eval runs
- [ ] Calibrate `LLMCostRouter` heuristics from those baselines
- [ ] Retest after major prompt changes
...
> **Diagram:** [Three-layer token budget stack](diagrams/08-budget-layers.html).
...
Different models have different context windows. `LLMCostRouter` now filters candidates by context window (see spec 06) so a task requiring 150 000 tokens of context is never routed to a model with a 100 000-token window.
