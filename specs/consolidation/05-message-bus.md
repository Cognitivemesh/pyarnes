...
> | **Status** | active |
> | **Type** | core-runtime |
> | **Tags** | messaging, bus, infra, persistence |
...
> | **Read before** | 06-model-router.md |
> | **Not owned here** | model selection (see `06-model-router.md`); provider config (see `13-provider-config.md`); transport adapters (see `12-transport.md`) — the bus is inter-agent messaging only, not model-call plumbing |
...
- [ ] **InMemoryBus** — for unit tests; no persistence, no inter-process visibility
- [ ] **TursoMessageBus** — for local development and small swarms; embedded MVCC DB
- [ ] **NatsJetStreamBus** — for production; requires a separate NATS server
